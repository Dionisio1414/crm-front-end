...
use App\Documents\Models\TransferStocks\DocumentTransferStock as Model;
use App\Documents\Models\TransferStocks\DocumentTransferStocksHeader;
 ...
 */
class TransferStockRepository extends CoreRepository
{
    ...
    public function getItemsTable()
    {
        ...
        $data = $this->getTable($data, DocumentTransferStocksHeader::class);
...
//        }
        $data = [
            ...
            'document_type' => $this->getDirectoryCore('documents', 4),
            'id' => $convert_id,
           ...
           //'amount' => $nomenclature_price_amount . ' ' . TitleJson::getArray($item->currency->title, $this->lang) ?? null,
            ...
            'warehouse' => optional($item->warehouse)->title . '||' . optional($item->to_warehouse)->title,
            ...
            //delivery
            'delivery' => $this->getDirectoryImageCore('type_deliveries', optional($item->delivery)->type_deliveries_id) ?? $this->getDirectoryCore('type_deliveries', optional($item->delivery)->type_deliveries_id),
            'ttn_number' => optional($item->delivery)->ttn_number ?? null,
            'ttn_status' => optional($item->delivery)->ttn_status ?? null,

        ];
        //dd($data);
        $data = $this->changeCellsCustomFormatBd($data, DocumentTransferStocksHeader::class);
        $item->cells()->delete();
    ...
    public function createOrUpdateDelivery($id)
    {
        if($this->request->delivery){
            ...
            $item->delivery()->updateOrCreate([
                'document_transfer_stock_id' => $item->id
    ...
    {
        $item = $this->modelVisible()->find($id);
        return $this->getOneTable($item, DocumentTransferStocksHeader::class);
    }
