...
import OrderTable from "@/components/dashboard/crm/Orders/OrderTable/OrderTable";
// Components async
const CreateSupplier = () => import("@/components/dashboard/products/suppliers/modal/CreateSupplier/CreateSupplier");
const OrderModal = () => import("@/components/dashboard/crm/Orders/modal/OrderModal/OrderModal");
...
export default {
  ...
  methods: {
    ...
    onTabValue(id) {
      switch (id) {
        case 2:
          this.$store.dispatch('getFilteredOrderTable', {point: true, text: ''});
          break;
        case 3:
          this.$store.dispatch('getFilteredOrderTable', {point: false, text: ''});
          break;
        default:
          this.$store.dispatch('getFilteredOrderTable', null);
          break;
      }
    },
    onStatusDocument() {
    ...
    onEditItem(values) {
      ...
      this.orderModal = TYPE_OF_STOCK_MODAL.POST;
    ...
    onCopyItem(values) {
      ...
      this.documentTitle = title;
      this.orderModal = TYPE_OF_STOCK_MODAL.PURCHASES;
      this.documentEditData = val || values;
    },
    onClickOutside () {
...
<style>
.order .v-data-table.table > .v-data-table__wrapper > table > tbody > tr > td {
  padding: 8px 30px;
}
</style>
