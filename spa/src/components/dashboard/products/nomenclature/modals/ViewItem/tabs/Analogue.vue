...
<div class="accompanying-header" v-if="isUpdate">
  <paginate-for-table
      ...
      :total-page="form.analog['total_page']"
...
<table-with-prices
    ...
    :body="form.analog.body"
    :headers="form.analog.headers"
...
export default {
  ...
  data() {
    return {
      ...
      currentPage: 1,
      resource: CHOOSE_MODAL_RESOURCES.ANALOGS
    }
  },
  props: {
    form: Object,
  ...
  methods: {
    ...
    onSelectItem(item) {
      const resource = CHOOSE_MODAL_RESOURCES.ANALOGS
      this.$emit('add-analog-item', item, resource)
    ...
    onToggleAll(isAllSelected) {
      if (isAllSelected) {
        ...
        this.selectedNomenclatures = this.form.analog.body.map(item => ({id: item.id}))
      }
    },
    onDeleteList() {
      return false
    ...
    onClickDelRow() {
      this.$emit('del-analog-rows', [...this.selectedNomenclatures])
      this.selectedNomenclatures = []
    },
    onClickChooseBtn() {
      this.$emit('click-choose-btn',
          {
            chooseResource: CHOOSE_MODAL_RESOURCES.ANALOGS
          }
      )
    },
  }
