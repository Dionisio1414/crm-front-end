            ...
            <th class="thTable-settings">
              <div class="table-settings" v-click-outside="hideOutsideSettings">
                ...
                <div class="settings-popup" v-if="settingsFlag">
                  ...
                  <draggable
                    ...
                    <transition-group type="transition" :name="!drag ? 'flip-list' : null">
                      <div v-for="(val, index) in settingsTable.header" :key="val.id" class="settings-list-item">
                        <div class="checkbox">
                          <label class="checkbox-label">
                            <!--                                  <input type="checkbox" @change="checkCheckedRemove($event, index)" :value="val.value" :checked="val.is_visible">-->
                  ...
                  <button type="button" class="base-btn darken-btn" @click="saveTableSettings">Сохранить</button>
                </div>
              </div>
            </th>
          </tr>
          ...
          <tr v-for="(val, index) in items[0]"
            ...
            <td>
              <div class="checkbox" @click.stop="(() => false)">
                <label class="checkbox-label">
                  <input v-if="!val.is_editable"
                         type="checkbox"
                         @change="selectItem(val.id, selected )"
                         value=""
                         :checked="selected"
                  >
                  <div class="checkbox-text" :class="{'disabled': !isCheckItem && !val.is_editable}"></div>
                </label>
              </div>
              <div v-if="!isCheckItem" :class="{'favorite': true, 'favorite-active': val.is_default}" @click.stop="setFavorite(val.id)">
            ...
            <td v-for="(value, key) in items[1]"
                :key="key"
                :ref="key">{{ val.cells[value.value] }}</td>
          </tr>
          </tbody>
        </template>
      </v-data-table>
      ...
      <div class="table-controls">
        <div class="controls-list" v-if="actions.length">
          <div class="controls-list-item" v-for="(val, index) in actions" :key="index">
            ...
            <button class="control-btn" :disabled="selectedItems.length !== 1" type="button" @click.prevent="onClickCopy" v-if="val.type === 'copy'">
              <span class="icon">
            ...
            <button :disabled="!selectedItems.length" class="control-btn" type="button" @click="onClickMove" v-if="val.type === 'move'">
              <span class="icon">
                <svg width="46" height="46" viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <circle opacity="0.8" cx="23" cy="23" r="23" fill="#9DCCFF"/>
                  <path d="M33.4109 22.1208L30.1462 19.5495C30.1189 19.5281 30.0861 19.5147 30.0515 19.511C30.017 19.5074 29.9821 19.5135 29.9508 19.5286C29.9196 19.5438 29.8932 19.5675 29.8749 19.5969C29.8565 19.6264 29.8468 19.6604 29.8468 19.6951V21.3471H24.1669V15.6785H25.8248C25.9783 15.6785 26.0653 15.4998 25.9707 15.3798L23.3917 12.1216C23.3746 12.0996 23.3527 12.0817 23.3276 12.0694C23.3026 12.0572 23.275 12.0508 23.2471 12.0508C23.2192 12.0508 23.1916 12.0572 23.1666 12.0694C23.1415 12.0817 23.1196 12.0996 23.1025 12.1216L20.5235 15.3798C20.5021 15.407 20.4887 15.4398 20.485 15.4743C20.4813 15.5088 20.4874 15.5436 20.5026 15.5748C20.5179 15.606 20.5416 15.6322 20.5711 15.6506C20.6005 15.6689 20.6346 15.6786 20.6694 15.6785H22.3247V21.3471H16.6448V19.6925C16.6448 19.5393 16.4657 19.4525 16.3454 19.547L13.0808 22.1208C13.0587 22.1378 13.0408 22.1597 13.0285 22.1847C13.0162 22.2097 13.0098 22.2372 13.0098 22.2651C13.0098 22.2929 13.0162 22.3204 13.0285 22.3454C13.0408 22.3704 13.0587 22.3923 13.0808 22.4093L16.3429 24.9832C16.4631 25.0776 16.6422 24.9934 16.6422 24.8376V23.1856H22.3222V28.8541H20.6643C20.5107 28.8541 20.4238 29.0329 20.5184 29.1529L23.0974 32.4085C23.1716 32.503 23.3149 32.503 23.3865 32.4085L25.9655 29.1529C26.0602 29.0329 25.9758 28.8541 25.8197 28.8541H24.1669V23.1856H29.8468V24.8402C29.8468 24.9934 30.0259 25.0802 30.1462 24.9857L33.4083 22.4119C33.4304 22.3946 33.4482 22.3726 33.4606 22.3475C33.4729 22.3224 33.4795 22.2949 33.4797 22.267C33.4799 22.239 33.4739 22.2113 33.462 22.186C33.4501 22.1607 33.4326 22.1384 33.4109 22.1208Z" fill="white"/>
                </svg>
              </span>
            </button>
      ...
      <li class="context-menu-item">
        <div class="item-icon">
          <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M17.8382 12.2973L16.0838 10.9128C16.0691 10.9012 16.0515 10.8941 16.0329 10.8921C16.0144 10.8901 15.9956 10.8934 15.9788 10.9015C15.962 10.9097 15.9479 10.9225 15.938 10.9383C15.9281 10.9542 15.9229 10.9725 15.923 10.9911V11.8807H12.8707V8.8284H13.7616C13.8441 8.8284 13.8908 8.73216 13.84 8.66754L12.4541 6.91315C12.4449 6.90128 12.4331 6.89166 12.4196 6.88505C12.4062 6.87844 12.3914 6.875 12.3764 6.875C12.3614 6.875 12.3466 6.87844 12.3331 6.88505C12.3196 6.89166 12.3079 6.90128 12.2987 6.91315L10.9128 8.66754C10.9012 8.68222 10.8941 8.69985 10.8921 8.71842C10.8901 8.73699 10.8934 8.75575 10.9015 8.77253C10.9097 8.78932 10.9225 8.80347 10.9383 8.81335C10.9542 8.82323 10.9725 8.82845 10.9911 8.8284H11.8807V11.8807H8.8284V10.9898C8.8284 10.9073 8.73216 10.8605 8.66754 10.9114L6.91315 12.2973C6.90128 12.3065 6.89166 12.3183 6.88505 12.3317C6.87844 12.3452 6.875 12.36 6.875 12.375C6.875 12.39 6.87844 12.4048 6.88505 12.4183C6.89166 12.4317 6.90128 12.4435 6.91315 12.4527L8.66616 13.8386C8.73079 13.8895 8.82703 13.8441 8.82703 13.7602V12.8707H11.8793V15.923H10.9884C10.9059 15.923 10.8592 16.0192 10.91 16.0838L12.2959 17.8368C12.3358 17.8877 12.4128 17.8877 12.4513 17.8368L13.8372 16.0838C13.8881 16.0192 13.8427 15.923 13.7588 15.923H12.8707V12.8707H15.923V13.7616C15.923 13.8441 16.0192 13.8908 16.0838 13.84L17.8368 12.4541C17.8487 12.4448 17.8583 12.4329 17.8649 12.4194C17.8716 12.4059 17.8751 12.3911 17.8752 12.376C17.8753 12.361 17.8721 12.3461 17.8657 12.3325C17.8593 12.3188 17.8499 12.3068 17.8382 12.2973Z" fill="white"/>
          </svg>

        </div>
        <a @click.prevent="onClickContextMove($event, data)">Перенести</a>
      </li>
      <li class="context-menu-item">
        <div class="item-icon">
...
export default {
  ...
  methods: {
    ...
    onClickChange() {
      const item = this.body.find(item => item.id === this.selectedItems[0]);
      const values = {
        val: item, title: MODE_TYPES.EDIT
      }

      this.$emit('editList', values)
      this.$emit('changeItem', JSON.parse(JSON.stringify(item)))
    },
    onClickMove() {
      const val = [...this.selectedItems];
      const values = {
    ...
    onClickCopy() {
      const item = this.body.find(item => item.id === this.selectedItems[0]);
      const values = {
        val: item, title: MODE_TYPES.COPY
      }
    ...
    onClickContextDelete(e, { val }) {
      const id = [val.id];
      const values = {
        val: id, title: MODE_TYPES.REMOVE
      }
    ...
    onClickContextCopy(e, { val }) {
      const values = {
        val, title: MODE_TYPES.COPY
      }

      this.$emit('copyList', values)
      console.log('Copy', e.target, val)
    },
    onClickRow(item) {
      const newItem = JSON.parse(JSON.stringify(item))
      this.$emit('clickRow', newItem)
    },
    clearChecked() {
      this.selectedItems = []
      this.selected = false
    ...
    // checkedRemoveColumns() {
    //     this.settingsTable
    //     this.$refs[currentColumn].forEach(item => item.style.display = 'none')
    // },
    // checkCheckedRemove({ target }, index) {
    //   const currentHeaderCell = this.settingsTable.header[index], currentColumn = currentHeaderCell.value
    //   if(!target.checked) {
    //     this.$refs[currentColumn].forEach(item => item.style.display = 'none')
    //     currentHeaderCell.is_visible = false
    //     this.transformHeadersHandler(this.settingsTable.header)
    //   } else {
    //     this.$refs[currentColumn].forEach(item => item.style.display = 'table-cell')
    //     currentHeaderCell.is_visible = true
    //     this.transformHeadersHandler(this.settingsTable.header)
    //   }
    // },
    async saveTableSettings() {
      ...
      try {
        await axios.put(`https://${domain}/api/v1/${this.resource}/headers`,
            {...data},
  ...
  created() {
    console.log('refs created', this.header)
  },
  mounted() {
