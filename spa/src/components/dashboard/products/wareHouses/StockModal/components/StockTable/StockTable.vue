<template>
  <div class="table-wrapper tableGoods" :class="customClass">
      ...
      >
        <template v-slot:header="{props}">
          ...
          <tr>
            ...
            <th :class="{'dots': !isCrossDelete}"></th>
          </tr>
          </thead>
        </template>
    ...
    <div class="table-footer">
      <div class="totalWrap">
        <span class="text">{{$t('page.wareHouses.stockModal.total')}} :</span>
        <div class="count">
          <span class="totalCount">{{ amountCount }}</span>
          <span v-if="isStock" class="totalSum">{{ amountSum | formatPrice }}</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...mapActions(['getSelectedProduct', 'getSelectedProductWriteOff']),
    async onChange({ nomenclature_id }) {
      ...
      if (this.isStock) {
        ...
        await this.getSelectedProductWriteOff(query);
      }
    ...
    onAmountCount(amount) {
      ...
      const counting = array.map(item => item)

      this.$emit('updateItems', counting);
    },
    toggleAll() {
      this.selectedItems = [];
      if (!this.selectedAll) {
