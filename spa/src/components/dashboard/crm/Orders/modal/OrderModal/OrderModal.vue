    ...
    >
      <template v-slot:header>
        <div class="dialog-header">
          ...
          <div v-if="amountSum" class="sum">{{ $t('orders.amountOfDeal') }}: <span class="count">{{ amountSum | formatPrice }}</span> {{currency}}</div>
          <header-actions
              id="purchases"
      ...
      <template v-slot:main>
        <div class="dialog-content-header" :class="{ 'disabledBlock': isCapitalizedDocument }">
          <div class="left">
            ...
            <v-select
                class="companies"
                :placeholder="$t('orders.selectCompany')"
        ...
        <simplebar  class="dialog-main">
          <form ref="form">
            <v-row>
              <v-col cols="3">
                <div class="select-wrap">
                  <div class="label-title">{{$t('orders.warehouse')}} *</div>
                  <v-select
                      ...
                      :items="wareHouses"
                      item-text="title"
                      :placeholder="$t('orders.warehousePlaceholder')"
              ...
              <v-col cols="3">
                <div class="select-wrap">
                  <div class="label-title">{{$t('orders.buyersTitle')}} *</div>
                  <v-select
                      class="select-switcher"
                      :class="{'error-on-select':  $v.form.supplier_id.$error }"
                      @blur="$v.form.supplier_id.$touch()"
                      :placeholder="$t('orders.buyersPlaceholder')"
                      :menu-props="{ contentClass: 'defaultDropDown resp'}"
                      :items="suppliers"
                      item-text="title"
                      item-value="id"
              ...
              <v-col cols="3">
                <div class="select-wrap">
                  <div class="label-title">{{$t('orders.contract')}} *</div>
                  <v-select
                      class="select-switcher"
                      :class="{'error-on-select':  $v.form.contract_id.$error }"
                      @blur="$v.form.contract_id.$touch()"
                      :placeholder="$t('orders.contractPlaceholder')"
                      :menu-props="{ contentClass: 'defaultDropDown resp'}"
                      item-text="title"
                      item-value="id"
                      :items="contracts"
              ...
              <v-col cols="1">
                <div class="select-wrap">
                  <div class="label-title">{{$t('orders.currency')}}</div>
              ...
              <v-col cols="2">
                <div class="select-wrap">
                  <div class="label-title">{{$t('orders.typeOfPrice')}}</div>
                  <v-select
                      class="select-switcher"
                      :class="{'error-on-select':  $v.form.contract_id.$error }"
                      @blur="$v.form.contract_id.$touch()"
                      :placeholder="$t('orders.typeOfPricePlaceholder')"
                      :menu-props="{ contentClass: 'defaultDropDown resp'}"
                      item-text="title"
                      item-value="id"
                      :items="contracts"
              ...
              <v-col cols="6">
                <v-row
                    ...
                    class="dark-row left">
                  <v-col cols="6">
                    <div class="label-title">{{$t('orders.dateOfReceiptPlanned')}}</div>
                    <calendar
                  ...
                  <v-col cols="6">
                    <div class="label-title">{{$t('orders.scheduledPaymentDate')}}</div>
                    <calendar
                        @focus="changeFocusRow('schedule_dates')"
                        @blur="checkFocusedRow('schedule_dates')"
                        @updateDate="onDate"
                ...
                <v-row class="dark-row right" :class="{'active' : focusRow === 'actual_dates'}">
                  <v-col cols="6">
                    <div class="label-title">{{$t('orders.dateOfReceiptActual')}}</div>
                    <calendar
                        @updateDate="onDate"
                        @focus="changeFocusRow('actual_dates')"
                        @blur="checkFocusedRow('actual_dates')"
                        :placeholder="$t('orders.chooseDate')"
                        :allowDate="form.date"
              ...
              <v-col cols="6">
                <v-row class="dark-row" :class="{'active': focusRow === 'status'}">
                  <v-col cols="6">
                    <div class="select-wrap">
                      <div class="label-title">{{$t('orders.status')}} *</div>
                      <v-select
                          class="select-switcher"
                          :placeholder="$t('orders.chooseStatus')"
                  ...
                  <v-col cols="6">
                    <div class="select-wrap">
                      <div class="label-title">{{$t('orders.statusOfPayment')}} *</div>
                      <v-select
                          class="select-switcher"
                  ...
                  <div class="checkbox">
                    <label class="checkbox-label">
                      ...
                      <span class="checkbox-text">
                          <span class="text">Закрыть заказ</span>
                      </span>
                    </label>
                  </div>
                  <div class="orderStatus">
                    <span>{{ isSuccess ? $t('orders.success') : $t('orders.fail') }}</span>
                    <div class="icon">
                      <svg-sprite width="22" height="22" :icon-id="isSuccess ? 'success': 'fail'"></svg-sprite>
                    </div>
                  </div>
                </v-col>
              </v-col>
              ...
              <v-col cols='12'>
                <div class="center">
                  <div class="centerHeader">
                    ...
                    <div class="right" v-if="!isCapitalizedDocument">
                      ...
                      <tooltip-with-icon
                          color="#F4F9FF"
                          :tooltip-text="$t('orders.addString')"
                      ...
                      <tooltip-with-icon
                          color="#F4F9FF"
                          customClass="delete"
                          :tooltip-text="$t('orders.deleteString')"
                      ...
                      <div class="discountBlock">
                        <div class="checkbox">
                          <label class="checkbox-label">
                            <input type="checkbox" :value="true">
                            <span class="checkbox-text">
                              <span class="text">{{ $t('orders.useDiscount') }}</span>
                            </span>
                          </label>
                        </div>
                        <div class="discountCounter">
                          <numeric-input
                              custom-class="ordersCountBtn"
                              @change="onInput"
                              :min="0"
                              :max="999"
                              :value="discountValue"
                ...
                <div class="stockTable" :class="{ 'disabledBlock': isCapitalizedDocument }" v-if="getHeaders">
                  ...
                  <div class="purchasesTotal">
                    <span class="text">{{$t('orders.total')}}:</span>
                    <div class="count">
              ...
              <v-col cols="6">
                <v-row
                  ...
                  <v-col cols="6">
                    <div class="select-wrap">
                      <div class="label-title">{{$t('orders.paymentTerm')}}</div>
                      <v-select
                          class="select-switcher"
                          :placeholder="$t('orders.paymentTermPlaceholder')"
                          :items="terms_payment"
                          :menu-props="{ contentClass: 'defaultDropDown resp'}"
                  ...
                  <v-col cols="6">
                    <div class="select-wrap">
                      <div class="label-title">{{$t('orders.formPayment')}}</div>
                      <v-select
                          class="select-switcher"
                          :placeholder="$t('orders.formPaymentPlaceholder')"
                          :menu-props="{ contentClass: 'defaultDropDown resp'}"
                          :items="form_payments"
                          @focus="changeFocusRow('payment')"
                          @input="checkFocusedRow('payment')"
              ...
              <v-col cols="6">
                <div class="checkbox">
                  <label class="checkbox-label">
                    <input type="checkbox" :value="true">
                    <span class="checkbox-text">
                      <span class="text">Доверенность</span>
                    </span>
                  </label>
                </div>
              </v-col>
            ...
            <v-row>
              <v-col cols="6">
                <div class="label-title">{{ $t('orders.comments') }}</div>
              </v-col>
              <v-col cols="6">
              </v-col>
              <v-col cols="6">
              ...
              <v-col cols="6" class="responsible">
                ...
                <div>{{ $t('orders.responsible') }}: <span>{{ getFullNameUser }}</span></div>
                <div>{{ $t('orders.dateCreateDocument') }}: <span> {{ form.document_date | formatDate }}</span></div>
              </v-col>
            </v-row>
...
// components
import OrderModalFirstTable from "@/components/dashboard/crm/Orders/modal/table/OrderModalFirstTable";
import OrderModalSecondTable from "@/components/dashboard/crm/Orders/modal/table/OrderModalSecondTable";
...
export default {
  ...
  components: {
    ...
    PopoverMenu,
    Calendar,
    OrderModalFirstTable,
    OrderModalSecondTable,
    ModalContainer,
    Annotation,
    NumericInput
  },
  ...
  computed: {
    ...
    getPurchasesModalTitle() {
      if (this.isCopy) {
        return `Закупки (${this.$t('orders.copy')})`;
      }
  ...
  data() {
    return {
      ...
      currency: '',
      priceType: '',
      tabsComponents: ['OrderModalFirstTable', 'OrderModalSecondTable'],
      editDelivery: null,
      list: [],
      randomId: '',
      date: getCurrentDate(),
