<template>
    <div v-frag>
        <div class="form-row">
            <v-row>
                <v-col cols="12">
                    <div class="label-title">{{ $t('user.addressLives') }}</div>
                </v-col>
            </v-row>
            <v-row>
                <v-col cols="4">
                ...
                <v-col cols="6">
                    <div class="form-field">
                        <input 
                            ...
                            type="text" 
                            placeholder="Улица" 
                            v-model="localData.residence_street"
                            @input="updateField($event, 'residence_street')"
                        >
                    </div>
                </v-col>
                <v-col cols="6">
                    <div class="form-field">
                        <input 
                            ...
                            v-model="localData.residence_number_house"
                            @input="updateField($event, 'residence_number_house')"
                        >
                    </div>
                </v-col>
            </v-row>
...
export default {
    ...
    methods: {
        ...
        async updateRegionPaginate(page) {
            ...
            await this.getAddressListWithPaginate({types, query: `?page=${page}&country_id=${this.localData.residence_country_id}`, paginate: true, page})
        },
        async updateCityPaginate(page) {
            ...
            await this.getAddressListWithPaginate({types, query: `?page=${page}&region_id=${this.localData.residence_region_id}`, paginate: true, page})
        },
        async onCountryValue(val) {
            ...
            this.localData.residence_country_id = val.id
            this.localData.residence_region_id = null
            this.localData.residence_city_id = null
        ...
        async onRegionValue(val) {
            ...
            this.localData.residence_region_id = val.id
        ...
        onCityValue(val) {
            ...
            this.settingsAutocomplete.city_id = val.id
            this.localData.residence_city_id = val.id
            this.$emit('updateData', { ...this.localData })
        },
    ...
    async created() {
        ...
        if(this.modeType === 'change') {
            await this.getAddressList({
                ...
                query: `?country_id=${this.addressData.residence_country_id}&selected_id=${this.addressData.residence_country_id}`
            })
            await this.getAddressList({
                types: {
                    type: 'organizations',
            ...
            await this.getAddressList({
                ...
                query: `?region_id=${this.addressData.residence_region_id}&selected_id=${this.addressData.residence_region_id}`
            })
            this.localData = { ...this.addressData }
            this.settingsAutocomplete.country_id = this.addressData.residence_country_id
            this.settingsAutocomplete.region_id = this.addressData.residence_region_id
            this.settingsAutocomplete.city_id = this.addressData.residence_city_id
        }
    },
    async mounted() {
        await this.getAddressList({
