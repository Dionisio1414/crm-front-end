      ...
      <div class="dialog">
        <div class="dialog-header">
          <div class="header-text">
            <span>{{ $t('directories.measurementsPoint') }}</span>
          </div>
          <div class="dialog-header-actions">
            <v-btn icon color="#5893D4" @click="addTab('units', 'directories.measurementsPoint')" :disabled="checkTabs" v-if="mode === 'create'">
        ...
        <div class="dialog-content dialog-content-large">
          <form class="item-form currency-form currency-form--add">
            <v-row class="mt-5">
              <v-col cols="3">
                <div class="item-name">
                  <div class="label-title">{{ $t('directories.shortName') }} <span class="mark-required">*</span>
                  ...
                  <input type="text"
                         :readonly=" mode === 'view'"
                         v-model="tableCells.title_short"
              ...
              <v-col cols="3">
                <div class="item-name">
                  ...
                  <input type="text"
                         :readonly=" mode === 'view'"
                         v-model="tableCells.title_full"
                         placeholder="Введите полное наименование">
                </div>
              </v-col>
              <v-col cols="3">
                <div class="item-name">
                  <div class="label-title">{{ $t('directories.internationalShort') }}</div>
                  <input type="text"
                         :readonly=" mode === 'view'"
                         v-model="tableCells.international"
                         placeholder="Введите международное сокращение">
                </div>
              </v-col>
              <v-col cols="3">
                <div class="item-name">
                  <div class="label-title">{{ $t('directories.code') }}</div>
                  <input type="text"
                         :readonly=" mode === 'view'"
                         v-model="tableCells.code"
                         placeholder="Введите код">
                </div>
              </v-col>
              <div v-if="mode !== 'view'" class="validation-rules mt-5">{{ $t('directories.validationRules') }}</div>
            </v-row>
          </form>
          <div class="dialog-actions popup-buttons">
            <v-btn
                v-if="mode !== 'view'"
                class="popup-btn transparent-btn base-button base-button--canc"
                color="#5893D4"
                text
                @click="cancel"
            >
            ...
            <v-btn
                ...
                color="#5893D4"
                light
                :disabled="!(tableCells.title_full && tableCells.title_short)"
                @click="onSave"
            >
              {{ $t('page.saveButton') }}
            </v-btn>
