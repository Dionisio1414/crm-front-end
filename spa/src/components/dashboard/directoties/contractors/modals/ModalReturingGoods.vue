    ...
    <template v-slot:header>
      <div class="dialog-header">
        <div class="header-text">
          <span>{{ $t('directories.reasonReturnText') }}</span>
        </div>
        <div class="dialog-header-actions">
          <v-btn icon color="#5893D4" @click="addTab('reason-returning-goods', 'directories.reasonReturnText')" :disabled="checkTabs" v-if="mode === 'create'">
                ...
                <v-col cols="6">
                    <div class="label-title label-title--required label-title--singular">
                      <div class="label-title-text">
                        {{ $t('directories.reasonReturnText') }}
                      </div>
                    ...
                    <div class="form-field">
                      <input 
                        ...
                        :readonly="mode === 'view'" 
                        v-model="tableCells.title" 
                        placeholder="Введите причину"
                      >
                      <div class="form-field-helper" v-if="mode === 'copy'">Измените название</div>
                    </div>
                </v-col>
              </v-row>
              <v-row>
...
export default {
  ...
  methods: {
    ...
    onCloseModal() {
      if(!this.loader) this.$emit('close')
    },
    onSave() {
      ...
      if(params) {
        this.$emit('save', {
          action: this.mode,
          resources: 'counterparties_returns',
          data: {...this.localFormData},
          alertItem: 'directories.reasonReturnText'
        })
        this.$router.go(-1)
    ...
    changeDefaultState() {
      ...
      if (this.mode !== 'create') {
        this.$emit('changeDefault', {id: this.localFormData.id, resources: 'counterparties_returns'})
      }
    },
    cancel() {
      if(!this.loader) {
