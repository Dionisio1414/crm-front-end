          ...
          <tbody>
            <template v-for="(val, index) in items[0]">
              ...
              >
                ...
                <template v-if="resource === 'counterparties'">
                    <router-link
                        class="link" 
                        tag="td" 
                        to="/directories/contractors/contractor/counterparty" 
                        v-for="(value, key) in items[1]" 
                        :key="key" 
                        :ref="key"
                    >
                        <span v-if="value.value === 'contact_detail'">
                        ...
                        <span v-else-if="value.value === 'document_issued_date' || value.value === 'document_validity_date'">
                            {{ val.cells[value.value] ? formatDate(val.cells[value.value].slice(0, 10)) : "" }}
                        </span>
                        <template v-else-if="value.value === 'is_supplier' || value.value === 'is_customer' || value.value === 'is_other'">
    ...
    <div class="table-controls">
      <div class="controls-list" v-if="actions.length">
          <div class="controls-list-item" v-for="(val, index) in actions" :key="index">
            ...
            <button :disabled="selectedItems.length !== 1" class="control-btn" type="button" @click="onClickCopy" v-if="val.type === 'copy'">
              <span class="icon">
...
export default {
  ...
  mounted() {
    // console.log('sorting items', this.sortSettingItems())
    // this.settingsTable.header.sort((a, b) => a.order - b.order)
  },
  directives: {
    ClickOutside
  }
