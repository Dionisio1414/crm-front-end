<template>
  <tr v-show="isShow" style="background-color: #F7F7F7"
      @contextmenu.prevent="openMenu($event)">
    <td>
      <div class="checkbox" @click.stop="(() => false)">
        <label class="checkbox-label">
          <input type="checkbox" @change="selectItem(item)" value="" :checked="selected">
    ...
    <td v-for="(value, key) in items[1]" :key="key" :ref="key" @click="selectItem(item)">
      <div v-if="value.value ==='is_visible'" class="table-switcher">
        <switcher @toggle="toggleVisible()" :value="item.cells[value.value]"></switcher>
      </div>
      <price-popup v-else-if="value.value.includes('price_')"
                   @clickPrice="onClickViewItem"
                   :price-value="item.cells[value.value]"
                   :options="item.cells[`${value.value}_option`]"
                   :price-name="value.value"
                   @changePrice="onChangePrice"/>
      <span class="value-wrapper" :class="{'short_title': value.value === 'short_title'}" v-else @click="onClickViewItem">{{ item.cells[value.value] }}</span>
    </td>
    <td></td>
    <td>
      <div class="nomenclature-actions">
        <span :class="{'active': isEditPurchase}">
          ...
          <button class="nomenclature-counter-submit"></button>
                ...
                <svg @click="isEditPurchase =!isEditPurchase" width="20" height="24" viewBox="0 0 20 24" fill="none"
        ...
        <span :class="{'active': isEditSales }">
          ...
          <button class="nomenclature-counter-submit"></button>
                ...
                <svg @click="isEditSales =!isEditSales" width="20" height="20" viewBox="0 0 20 20" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
                <path
...
export default {
  name: "NomenclatureGroupsProductRow",
  components: {PricePopup, Switcher, ContextMenu},
  props: ['items', 'item', 'parentId', 'selected', 'actions', 'isOpenGroup'],
  data() {
    return {
  ...
  computed: {
    isShow() {
      const currentItem = this.isOpenGroup.find(item => item.id === this.parentId);
      if (currentItem) {
        const { id, value } = currentItem;
        return value && (id === this.parentId);
      } else {
        return false;
      }
    ...
    formattedActions() {
      return this.actions.map(action => {
        if (action.action === TABLE_ACTIONS.MOVE) {
          return {
  ...
  mounted() {
    ...
    eventBus.$on('selectedAllChildNomenclatures', ({id, isAllSelected}) => {
      const isThisGroup = id === this.parentId
      if (isThisGroup && !isAllSelected && !this.selected) {
        this.selectItem()
      }
      if (isThisGroup && isAllSelected && this.selected) {
        this.selectItem()
  ...
  methods: {
    ...
    onClickChangeItem(item) {
      this.$emit('changeItem', item, this.parentId)
    },
